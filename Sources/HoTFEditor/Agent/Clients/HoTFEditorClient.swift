...
struct HoTFEditorClient: AgentClient {
    ...
    private func run(
        ...
        guard let baseURL = BackendConfig.convexHttpURL else {
            throw HoTFEditorClientError.upstream("Backend not configured")
        }
        ...
        guard let session = await Clerk.shared.session, session.status == .active else {
            throw HoTFEditorClientError.unauthenticated
        }
        ...
        if let http = response as? HTTPURLResponse, http.statusCode >= 400 {
            ...
            throw HoTFEditorClientError.from(status: http.statusCode, body: body)
        }
...
enum HoTFEditorClientError: LocalizedError {
    ...
    static func from(status: Int, body: String) -> HoTFEditorClientError {
