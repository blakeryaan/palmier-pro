...
/// Writes a self-contained `.hotf` package: every resolvable media reference is copied
/// into the new bundle's `media/` directory and rewritten to a project-relative source
enum HoTFEditorProjectExporter {
    ...
    static func export(
        ...
        let staging = fm.temporaryDirectory.appendingPathComponent("hotfeditor-export-\(UUID().uuidString)", isDirectory: true)
