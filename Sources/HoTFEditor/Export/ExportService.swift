...
final class ExportService {
    ...
    /// Writes a self-contained `.hotf` bundle (all media collected internally).
    @discardableResult
    func exportHoTFEditorProject(
        timeline: Timeline,
        manifest: MediaManifest,
        generationLog: GenerationLog,
        sourceProjectURL: URL?,
        outputURL: URL
    ) async -> HoTFEditorProjectExporter.Report? {
        isExporting = true
        progress = 0
        error = nil
        ...
        do {
            Log.export.notice("hotfeditor export start url=\(outputURL.lastPathComponent)")
            let report = try await Task.detached(priority: .userInitiated) {
                try HoTFEditorProjectExporter.export(
            ...
            progress = 1.0
            Log.export.notice("hotfeditor export ok collected=\(report.collected.count) missing=\(report.missing.count)")
            return report
        } catch {
            self.error = Self.detailedMessage(for: error)
            Log.export.error("hotfeditor export failed: \(Self.detailedMessage(for: error))")
            return nil
        }
    }
