...
@Suite("HoTF Editor project export (collect media)")
struct HoTFEditorProjectExportTests {
    ...
    private func makeFixture() throws -> (root: URL, source: URL, dest: URL, externalContents: String) {
        ...
        let source = root.appendingPathComponent("Source.hotf", isDirectory: true)
        ...
        return (root, source, root.appendingPathComponent("Packaged.hotf", isDirectory: true), externalContents)
    }
    ...
    @Test func collectsExternalAndInternalRewritesSourcesReportsMissing() throws {
        ...
        let report = try HoTFEditorProjectExporter.export(
